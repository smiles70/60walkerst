...
| 25 | **Empty `screeningReport.employers` renders a broken Employment card** | Sawyer's Employment card showed empty because `employers: []` — UI renders title with no content | Always populate `screeningReport.employers` from `applicationDetails.jobs` or omit the card via conditional rendering |
| 26 | **`inquiries` field is typed as `number` — strings cause TypeScript errors** | Alaina had `inquiries: "-"` which compiled only because of loose typing; changing it to `0` broke user expectations | Use `0` for no inquiries; never use `"-"` for numeric fields — update interface if strings are genuinely needed |
| 27 | **Documents must live in `public/` AND URLs must point to them** | Sawyer's attachments had `url: "#"` even though files existed in `docs/` — downloads went nowhere | Copy docs to `public/` immediately, update `attachments[].url` to absolute paths like `/filename.ext` |
| 28 | **Credit score bar must match reference design exactly** | Sawyer's bar was solid green; reference shows rainbow gradient (red→yellow→green) with arrow | Before declaring UI done, compare rendered output side-by-side with user's reference screenshot |
| 29 | **`applicationDetails.jobs` and `screeningReport.employers` are separate fields** | Populated one but not the other — Employment card pulled from `screeningReport`, not `applicationDetails` | When adding applicant, cross-check that job data appears in BOTH `applicationDetails.jobs` and `screeningReport.employers` |
